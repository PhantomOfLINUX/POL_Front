// components/successAlert.tsx
interface AlertProps {
    title: string;
    message: string;
    onClose: () => void;
}

const DangerAlert = ({ title, message, onClose }: AlertProps) => {
    return (
        <div className="bg-danger-200 border-t-4 border-danger-500 rounded-b text-gray-900 px-4 py-3 shadow-md w-2/5 mt-10" role="alert">
            <div className="flex">
                <div className="py-1">
                    <svg className="fill-current h-6 w-6 text-danger-500 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                        <path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm12.73-1.41A8 8 0 1 0 4.34 4.34a8 8 0 0 0 11.32 11.32zM9 11V9h2v6H9v-4zm0-6h2v2H9V5z"/>
                    </svg>
                </div>
                <div>
                    <p className="font-bold">{title}</p>
                    <p className="text-sm">{message}</p>
                </div>
                <div className="ml-auto">
                    <button
                        className="bg-transparent border-0 text-black opacity-50 float-right text-3xl leading-none font-semibold outline-none focus:outline-none"
                        onClick={onClose}
                    >
                        <span className="text-black opacity-50 h-6 w-6 text-2xl block outline-none focus:outline-none">
                            ×
                        </span>
                    </button>
                </div>
            </div>
        </div>
    );
};

export default DangerAlert;