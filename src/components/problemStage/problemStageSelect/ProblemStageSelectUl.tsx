...
const ProblemStageSelectUl:React.FC<ProblemStageSelectUlType> = ({list,selectName,ulToggle,setUlToggle}) => {
    ...
    return (
        <ul ref={ulRef} className={`${ulToggle?"block" : "hidden"} bg-white relative top-2 min-w-24 items-center justify-between px-1 border-solid border rounded-md border-SelectBorder-color`}>
            {list.map(li=>{
