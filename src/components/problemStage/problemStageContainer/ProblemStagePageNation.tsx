...
const ProblemStagePageNation:React.FC<ProblemStagePageNationType> = ({totalStages}) => {
    ...
    return (
        <div className="flex">
            <ProblemStagePageNationBtn pageMove={()=>{ChangeChecked(totalPages[curPageNation-1][0]); setCurPageNation(pre=>pre-1)}} checked={curPageNation===0} pageBtn={"<"}/>
                {totalPages[curPageNation]?.map(ele=><ProblemStagePageNationBtn pageMove={()=>{ChangeChecked(ele)}} curPage={searchParams.get("page")?Number(searchParams.get("page")):1} key={ele} pageBtn={ele}/>)}
            <ProblemStagePageNationBtn pageMove={()=>{ChangeChecked(totalPages[curPageNation+1][0]); setCurPageNation(pre=>pre+1)}} checked={curPageNation===totalPages.length-1} pageBtn={">"}/>
        </div>
    )
}
